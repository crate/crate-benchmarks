-- Registers the local filesystem repository holding the `uservisits`
-- snapshot. Used by select/count_distinct_destinationURL.toml, which restores
-- the table instead of re-importing it with COPY FROM.
--
...
-- long-running instance, DROP REPOSITORY uservisits_snapshots first.
CREATE REPOSITORY uservisits_snapshots TYPE fs WITH (
    location = '/home/haris/projects/crate/test-data/snapshots/uservisits'
);
